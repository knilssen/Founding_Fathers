    ...
    if article == "ERROR WITH DOWNLOADING ARTICLE TEXT":
        ...
        for itemPosition in range(0,len(taged)-1):
            ...
            if firstItem:
                ...
                # Test if item is position title
                if item[0].lower() in positionTitles:
                    # If next item is a person tag, ignore and let case:2 handle it, if not lets do it our selfs.
                    # Temporary fix to Named Entity Recognizer not recognizing an instant where Senator Frockt, was not tagged as a person but problem,” Frockt said.
                    #       Was tagged as a person, thus not entering into found person based off of no previous found or mentioned name
                    if taged[itemPosition + 1][1] != 'PERSON':
                        # Make the thing following the tag a person, if its not it wont matter
                ...
                if item[1] == Type:
                    ...
                    # SubCase: if person has multiple last or first names, keep on checking for more name tags.
                        ...
                        print "1", taged[itemPosition -1][0].lower(), item[0].lower().encode("utf-8")
        ...
        if len(article_people) >= 1:
            ...
            try:
                print "\n", "!!! Where it would be entered into database, but not since this is a test !!!", "\n"
                article_id = 69 # Tee-Hee
...
def main(Urls, Keywords, otherNames, total_article_count):
    ...
    for y, Url in enumerate(article_dict):
        ...
        # Check duplicate is equal to zero so that articles that are in our database are still able to be tested on

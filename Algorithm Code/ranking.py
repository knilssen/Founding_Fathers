...
def main():
    ...
    try:
        ...
        cursor.execute(("SELECT * FROM article_person"))
        article_person_link_data = cursor.fetchall()
        # print article_person_link_data
        for link in article_person_link_data:
            total_shares = 0
            cursor.execute(("SELECT * FROM article_based_weights WHERE articles_id = '%s'") % (link[0]))
            article_based_weights_data = cursor.fetchall()[0]
            cursor.execute(("SELECT * FROM social_media WHERE article_based_weights_articles_id = '%s'") % (link[0]))
            social_media_data = (cursor.fetchall())[0]
            cursor.execute(("SELECT post_date, url, source, keywords, summary FROM articles WHERE id = '%s'") % (link[0]))
            articles_data = (cursor.fetchall())[0]
            cursor.execute(("SELECT first_name, last_name FROM people WHERE id = '%s'") % (link[1]))
            ...
            add_score = ("UPDATE article_person SET "
                   "weight=%s "

...
import olympian_update_all
import downloader_olympian
...
class article_worker(Thread):
   ...
   def run(self):
       while True:
           ...
           Url, Source, art_id = self.queue.get()
           try:
               ...
               print "ERROR WITH DOWNLOADING ARTICLE TEXT FROM:", art_id, Url
               article.text = "ERROR WITH DOWNLOADING ARTICLE TEXT"
           finally:
               self.article_dict[Url] = [Source, article, art_id]
    ...
    if article == "ERROR WITH DOWNLOADING ARTICLE TEXT":
        ...
        if len(article_people) >= 1:
            ...
            try:
                ...
                olympian_update_all.main(art_id, article.top_image, submitted_v_articleText)
                mysql_article_person_link.main(art_id, article_people, totoltypecount)
                mysql_article_based_weights.main(art_id, len(articleText), "yes")
            ...
            print "        ", found_article_number, (3-len(str(found_article_number))) * " " + "                  ", art_id, (3-len(str(art_id))) * " " + "                  ", output_people
...
def main(Urls, Keywords, otherNames, total_article_count):
    ...
    # for source in source_name:
            queue.put((Url[1], "The Olympian", Url[0]))
